#!/usr/bin/env python3
"""
train_non_gate_verifiers.py — trener fire binære andretrinnsmodeller:

  "er dette virkelig en ball valve?"
  "er dette virkelig en globe valve?"
  "er dette virkelig en check valve?"
  "er dette virkelig en butterfly valve?"

Hovedmodellen model_cnn.pt endres IKKE. Gate open/closed beholder derfor
samme vekter og samme inferensløp. Andretrinnet brukes bare når hovedmodellen
allerede har foreslått en av de fire nye klassene.
...
Bruk:
  py train_non_gate_verifiers.py --real dataset --synth synth ^
      --holdout-list "25VHO64PU00101,25VHO71PW00101,25WHO71PW00101"

Ut:
  non_gate_verifiers.pt
"""
import argparse
import glob
import os
...
TARGETS = ("ball_valve", "globe_valve", "check_valve", "butterfly_valve", "reducer")
DIR_TO_LABEL = dict(REAL_MAP)
DIR_TO_LABEL["GateValve"] = "gate_valve"  # kun negativt i disse binærmodellene
...
def collect_synthetic(root):
    ...
    for label in sorted(os.listdir(root)):
        ...
        for fp in glob.glob(os.path.join(d, "*.png")):
            items.append((fp, label, "synthetic"))
    return items
...
def main():
    ...
    ap.add_argument("--model", default="non_gate_verifiers.pt")
    args = ap.parse_args()
    ...
    print("    model_cnn.pt er ikke endret; gate valve-løpet er urørt.")
